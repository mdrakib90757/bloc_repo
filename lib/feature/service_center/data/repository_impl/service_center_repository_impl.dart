...
class ServiceCenterRepositoryImpl implements ServiceCenterAddButtonRepo,fetchServiceCenterAddButtonRepo{
  final ServiceCenterApi serviceCenterApi;
  ServiceCenterRepositoryImpl(this.serviceCenterApi);

  @override
Future<void>createServiceCenterAddButton(String companyId,AddButtonRequest request)async{
  try{
    final response = await serviceCenterApi.createServiceCenterAddButton(request,companyId);
    if(response.statusCode!=200){
      throw Exception("Failed to create service center. Status: ${response.statusCode}, Data: ${response.data}");
    }
  }on DioException catch(e){
    throw Exception(e.response?.data ?? "Unknown error during service center creation");
...
Future<ServiceCenterModel>fetchServiceCenterAddButton(String companyId)async{
  try{
    final serviceCenter  = await serviceCenterApi.fetchServiceCenterAddButton(companyId);
